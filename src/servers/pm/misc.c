...
int do_create_semaphore(void) //also with close semaphore
{
    int desc = m_in.m1_i1;
    int mystatus = m_in.m1_i2;
    int myclose = m_in.m1_i3;

    if (mystatus == 0 && myclose == 0) { //CREATE AND NOT CLOSE
        if (mp->ptr[desc] == NULL)  {
            mp->ptr[desc] = (struct Semaphore *) malloc (sizeof(struct Semaphore));
	    mp->ptr[desc]->count = 1;  //occupy resource
	    mp->ptr[desc]->wakeups = 0;
	    mp->ptr[desc]->ref_count = 0;
	    mp->ptr[desc]->front = 0;
	    mp->ptr[desc]->end = 0;
	    mp->ptr[desc]->desc = desc;
	    printf("Semaphore created with address %p\n", (void *) mp->ptr[desc]);
            printf("with count %d and wakeups %d and  ref_count %d\n", (int)mp->ptr[desc]->count, (int)mp->ptr[desc]->wakeups, (int)mp->ptr[desc]->ref_count);
   ...
   } //close mystatus and myclose
   else if (mystatus == 1 && myclose == 0) { //INHERITED AND NOT CLOSE

    //code in do_fork
    printf("Semaphore inherited with address %p\n", (void *) mp->ptr[desc]);
    mp->ptr[desc]->ref_count += 1;
    printf("with count %d and wakeups %d and ref_count %d\n", (int)mp->ptr[desc]->count, (int)mp->ptr[desc]->wakeups, (int)mp->ptr[desc]->ref_count);
   }

    if (myclose == 1) { // CLOSEING SEMAPHORE

	if (mp->ptr[desc]->ref_count == 0) {
            printf("Closing semaphore completely\n");
            free(mp->ptr[desc]);
	    mp->ptr[desc] = NULL;
        }

	else {
            printf("Closing semaphore for this process\n");
            mp->ptr[desc]->ref_count--;
	    //printf("ref_count value %d\n",mp->ptr[desc]->ref_count);
            mp->ptr[desc] = NULL;
	}
    }
    return(OK);
...
void push(struct Semaphore *ptr, int num)
{
   ptr->queue[ptr->end] = num;
   ptr->end = (ptr->end + 1) % NR_PROCS;
}
...
int pop(struct Semaphore *ptr) 
{
    int foo = ptr->queue[ptr->front];
    ptr->queue[ptr->front] = 0;
    ptr->front = (ptr->front + 1) % NR_PROCS;
    return foo;
}
...
{
    int desc = m_in.m1_i1; //file descriptor
    int myup = m_in.m1_i2; //argument received by system call 

    if (myup == 0) { //down operation

        if (mp->ptr[desc] != NULL) {    //check if the process has access to semaphore

            if (mp->ptr[desc]->count == 1) {  // decrease the semaphore count

                mp->ptr[desc]->count = 0;
	        printf("Down operation successful\n");
	    }

	    else {  //then somebody else is using semaphore

                push(mp->ptr[desc], who_p);      //store process in a queue
		mp->ptr[desc]->wakeups += 1;     //increment the wakeups 
		printf("Process block itself\n"); 
		return(SUSPEND);   //do not reply
            } //close else 
        } //close if 
         ...
         if(mp->ptr[desc] != NULL) {  //check if the process has access to semaphore
             ...
             if(mp->ptr[desc]->count == 1) {   //no need to increment semaphore
                 printf("Process is already up\n");
	     }

	     else if (mp->ptr[desc]->count == 0 && mp->ptr[desc]->wakeups == 0) { //no pending processes
                 mp->ptr[desc]->count = 1;
		 printf("Up operation successful\n");
	     }

	     else {

                 int process_index = pop(mp->ptr[desc]);
		 setreply(process_index, OK); //unblock the process
		 mp->ptr[desc]->wakeups--;
		 printf("Up operation successful. Process unblocked\n");
	         //printf("Up operation successful\n");
	     }
	 }
    }
